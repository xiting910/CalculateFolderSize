...
public sealed partial class ResultViewModel : ObservableObject
{
        ...
        FolderSize rootSize)
    {
        ...
        else
        {
            ...
            _toast.Show("根文件夹的子项缓存已失效, 请重新计算后再打开结果视图");
        }
        Refresh();
    }
    ...
    /// <summary>
    /// 是否显示打开文件夹按钮, 安卓端无系统文件资源管理器, 恒为 <see langword="false"/>
    /// </summary>
    public bool ShowOpenFolderButton => !OperatingSystem.IsAndroid();
    ...
    /// <summary>
    /// 请求关闭当前结果视图的事件, 由壳视图模型处理
    /// </summary>
    public event Action<ResultViewModel>? CloseRequested;
    ...
    public void NavigateDown(ResultItemViewModel item)
    {
        ...
        {
            _toast.Show("该文件夹的子项数据已不在缓存中, 请重新计算该文件夹");
            return;
        }
    ...
    /// 通过面包屑跳转到指定层级
    /// </summary>
    /// <param name="index">面包屑段索引, 0 为第一段</param>
    public void NavigateToBreadcrumb(int index)
    {
        var rootSegmentCount = RootPath.Split(
            Constants.AllowedDirectorySeparators,
            StringSplitOptions.RemoveEmptyEntries
        ).Length;
        var targetLayer = Math.Max(index - rootSegmentCount + 2, 1);

        var popCount = _navigationStack.Count - (targetLayer - 1);
